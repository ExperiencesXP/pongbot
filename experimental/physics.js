const PongPhysics = {
  MAX_SPEED: 1800,
  UNCAP_HITS: 24,                 // after this many paddle hits, drop the cap
  MIN_SPEED: 320,
  SERVE_SPEED: 380,
  SPEED_UP: 1.07,                 // multiply speed on paddle hit
  WALL_SPEED_UP: 1.02,            // walls speed up much less than paddles
  PADDLE_ANGLE: (72 * Math.PI) / 180, // max angle from horizontal via hit offset
  WALL_NOISE: (11 * Math.PI) / 180,   // random extra angle on wall bounce
  PADDLE_NOISE: (13 * Math.PI) / 180, // random extra angle on paddle bounce
  SPIN_ON_HIT: 2.4,
  SPIN_ON_MOTION: 0.0018,
  SPIN_WALL: 0.28,
  SPIN_DECAY: 1.6,                // per-second exponential decay
};
