...
function bindMenu() {
  ...
  fillSelect(leftSel, presets, window.LEFT_AI || 'ace');
  fillSelect(rightSel, presets, window.RIGHT_AI || 'perfect');
...
function thinkTarget(paddle, opponent, dt) {
  ...
  if (out && out.moveSpeed != null && isFinite(out.moveSpeed)) {
    paddle.moveSpeed = out.moveSpeed;
  } else {
...
function movePaddle(paddle, dt) {
  ...
  let topSpeed = paddle.speed;
  if (paddle.ai && paddle.ai.funSpeed) topSpeed = paddle.ai.funSpeed;
  else if (paddle.ai && paddle.ai.id === 'trickster') topSpeed = 540;
  if (paddle.moveSpeed != null) topSpeed = Math.min(topSpeed, Math.max(0, paddle.moveSpeed));
  const maxMove = topSpeed * dt;
